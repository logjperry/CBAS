...
class Features(Dataset):
    def __init__(self, data_dir, transform=None, bin_size=8):
        """
        Args:
...
def train_autoencoder(data_loader, seq_len=16):
    ...
    m_path = f'C:\\Users\\Jones-Lab\\Documents\\cbas_test\\fibonacci_encoder_{seq_len}.pth'
...
def load_data(recording_path):
    ...
    # Create a DataLoader
    data_loader = DataLoader(dataset, batch_size=256, shuffle=True, num_workers=4)
...
def infer(config_path, recording_path):
    ...
    for path in deg_feature_paths:
        ...
        csv_path = os.path.splitext(path)[0]+'_'+str(model_config['seq_length'])+'_fibonacci_'+'encoded'+'.csv'
