...
class Features(Dataset):
    def __init__(self, data_dir, transform=None, multiple=False, spacer=1):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        random_i = random.randint(2*self.spacer, len(data)-3*self.spacer)

        X = [data[random_i-2*self.spacer,:],data[random_i-1*self.spacer,:],data[random_i,:],data[random_i+1*self.spacer,:],data[random_i+2*self.spacer,:]]

        avg = (data[random_i-2*self.spacer,:]+data[random_i-1*self.spacer,:]+data[random_i,:]+data[random_i+1*self.spacer,:]+data[random_i+2*self.spacer,:])/5
        
        Y = [data[random_i-2*self.spacer,:]-avg,data[random_i-1*self.spacer,:]-avg,data[random_i,:]-avg,data[random_i+1*self.spacer,:]-avg,data[random_i+2*self.spacer,:]-avg]
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, seq_len=5):
...
class TransformerEncoder(nn.Module):
    def __init__(self, input_dim, latent_dim, d_model, nhead, num_encoder_layers, seq_len):
        ...
        self.encoder_to_latent1 = nn.Linear(d_model, d_model)  
        self.encoder_to_latent2 = nn.Linear(d_model, latent_dim)  
    ...
    def forward(self, src):
        ...
        src = self.encoder_to_latent1(F.relu(src))
        src = self.dropout(src)
        src = self.encoder_to_latent2(F.relu(src))
...
class TransformerDecoder(nn.Module):
    def __init__(self, latent_dim, output_dim, d_model, nhead, num_decoder_layers, seq_len):
        ...
        self.unflatten = nn.Unflatten(1, (seq_len, latent_dim))  
    ...
    def forward(self, latent):
        ...
        # Expand from latent space
        tgt = self.unflatten(latent)

        tgt = self.latent_to_decoder(tgt)

        # Apply a positional encoding
...
def train_autoencoder(us_data_loader, seq_len=5, spacer=1):
    ...
    m_path = f'C:\\Users\\Jones-Lab\\Documents\\cbas_test\\encoder_'+str(spacer)+'.pth'
    ...
    decoder = Decoder(latent_dim, input_dim, d_model, nhead, num_decoder_layers, seq_len)
...
def load_data(recording_path, spacer):


    us_dataset = Features(data_dir=recording_path, multiple=True, spacer=spacer)

    # Create a DataLoader
    us_data_loader = DataLoader(us_dataset, batch_size=1024, shuffle=True, num_workers=8)

    train_autoencoder(us_data_loader, seq_len=5, spacer=spacer)
