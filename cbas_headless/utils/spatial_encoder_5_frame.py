...
class Features(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        X = np.array(data[(random_i-adj_binsize):(random_i+adj_binsize+1):5,:])
...
def train_autoencoder(data_loader, seq_len=5):
    ...
    m_path = f'C:\\Users\\Jones-Lab\\Documents\\cbas_test\\spatial_encoder_{seq_len}_frame_2fps.pth'
...
def infer(config_path, recording_path):
    ...
    for path in deg_feature_paths:
        ...
        for i in range(adj_binsize, len(data)-adj_binsize):
            X.append(np.array(data[(i-adj_binsize):(i+adj_binsize+1):5,:]))
        ...
        csv_path = os.path.splitext(path)[0]+'_'+str(model_config['seq_length'])+'_'+'encoded'+'_2fps'+'.csv'
