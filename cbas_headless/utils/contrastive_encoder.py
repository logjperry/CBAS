...
class S_Features(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for b, inst_num in data_insts:
            ...
            if start >= self.indices[-1]+1 and end < len(feats)-self.indices[-1]-2:
                random_i = random.randint(start, end-1)


            X = []

            X = [(feats[random_i+j-1,:]+feats[random_i+j,:]+feats[random_i+j+1,:])/3 for j in reverse]
...
class US_Features(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        random_i = random.randint(self.indices[-1]+1, len(data)-self.indices[-1]-2)

        X = [(data[random_i+j-1,:]+data[random_i+j,:]+data[random_i+j+1,:])/3 for j in reverse]
...
def train_autoencoder(us_data_loader, s_data_loader, seq_len=16):
    ...
    m_path = f'C:\\Users\\Jones-Lab\\Documents\\cbas_test\\contrastive_encoder.pth'
    ...
    optimizer_encoder = torch.optim.Adam(encoder.parameters(), lr=lr*.01)

    criterion_total = nn.MSELoss()
    criterion_encoder = nn.TripletMarginLoss(margin=5.0, p=2)
    ...
    for epoch in range(num_epochs):
        ...
        if False:
            for anchor, positive, negative in s_data_loader:
                ...
                positive = encoder(positive)
                negative = encoder(negative)


                loss = criterion_encoder(anchor, positive, negative)
                ...
                EL.append((epoch, loss.cpu().detach().numpy()))

                del anchor  # Delete the GPU tensor
                del positive  # Delete the GPU tensor
                del negative  # Delete the GPU tensor
...
def load_data(recording_path, instance_paths):
    ...
    train_autoencoder(us_data_loader, s_data_loader, seq_len=9)
...
def infer(config_path, recording_path):
    ...
    for path in deg_feature_paths:
        ...
        data = features
        X = []
        for i in range(indices[-1]+1, len(data)-indices[-1]-2):
            X.append([(data[i+j-1,:]+data[i+j,:]+data[i+j+1,:])/3 for j in reverse])
        ...
        for i in range(0, indices[-1]+1):
            total.append(y[0])
        for i in range(indices[-1]+1, len(data)-indices[-1]-2):
            total.append(y[i-indices[-1]-1])
        for i in range(len(data)-indices[-1]-2, len(data)):
            total.append(y[-1])
        ...
        csv_path = os.path.splitext(path)[0]+'_contrastive_'+'encoded'+'.csv'
