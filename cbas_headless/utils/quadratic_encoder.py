...
class Features(Dataset):
    def __init__(self, data_dir, transform=None, bin_size=3):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        if self.bin_size>0:
            for i in range(1, self.bin_size+1):
                indices.append(np.power(2, i))
        else:
            raise Exception('Bin size must be at least 1.')
        
        reverse = [i*-1 for i in indices]
        ...
        for k in range(0,len(indices)-1):
            forward.append(np.mean(data[random_i+indices[k]+1:random_i+indices[k+1],:], axis=0))

            
        backward = []
        for k in range(0,len(indices)-1):
            backward.append(np.mean(data[random_i+reverse[k+1]+1:random_i+reverse[k],:], axis=0))

        forward = np.array(forward)
        backward = np.array(backward)

        dif = np.subtract(forward,backward)
...
def train_autoencoder(data_loader, seq_len=16):
    ...
    m_path = f'C:\\Users\\Jones-Lab\\Documents\\cbas_test\\quadratic_encoder_{seq_len}.pth'
...
def infer(config_path, recording_path):
    ...
    for path in deg_feature_paths:
        ...
        # make the indices
        indices = [0]
        bin_size = model_config['seq_length']
        ...
        for i in range(indices[-1], len(data)-indices[-1]-1):
            ...
            forward = []
            for k in range(0,len(indices)-1):
                forward.append(np.mean(data[i+indices[k]+1:i+indices[k+1],:], axis=0))

                
            backward = []
            for k in range(0,len(indices)-1):
                backward.append(np.mean(data[i+reverse[k+1]+1:i+reverse[k],:], axis=0))
