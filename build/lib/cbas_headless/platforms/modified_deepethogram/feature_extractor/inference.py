...
from ..postprocessing import get_postprocessor_from_cfg
...
def extract(rgbs: list,
    ...
    real_vids = []
    for r in rgbs:
        basename = os.path.splitext(r)[0]
        stats = os.path.join(os.path.split(basename)[0],'stats.yaml')
        # make the outputfile have the same name as the video, with _outputs appended
        h5file = basename + '_outputs.h5'
        # check for file safety and for already inferenced
        if not os.path.isfile(stats) or os.path.isfile(h5file):
            continue
        real_vids.append(r)
        
    for i in tqdm(range(len(real_vids))):
        rgb = real_vids[i]
...
def feature_extractor_inference(cfg: DictConfig):
    ...
    #log.info('weights: {}'.format(feature_extractor_weights))
    ...
    # Added this to keep the model in memory during CBAS inference
    return (model, cfg.feature_extractor.final_activation, thresholds, postprocessor, cfg.augs.normalization.mean, cfg.feature_extractor.fusion, input_images, latent_name, device, cpu_transform, gpu_transform, cfg.inference.ignore_error, cfg.inference.overwrite, class_names, cfg.compute.num_workers, cfg.compute.batch_size)
